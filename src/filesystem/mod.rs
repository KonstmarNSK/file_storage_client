...
use self::fuse::BackgroundSession;
...
struct HelloFS {
    server_storage: Mutex<Box<dyn storage::Storage>>
}

unsafe impl Send for HelloFS{}
...
pub fn mount(mountpoint: &OsString, server_storage: Box<dyn Storage>) -> BackgroundSession<'static> {
    let bg_session;
    unsafe {
        bg_session = fuse::spawn_mount(
            HelloFS {
                server_storage: Mutex::new(server_storage)
            },
            mountpoint,
            &[])
            .unwrap();
